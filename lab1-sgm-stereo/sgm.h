...
namespace sgm
{
  ...
  public:
    SGM(unsigned int disparity_range, bool adaptive_penalties = false, unsigned int p1=3, unsigned int p2=40,
        float conf_thresh=20.0, unsigned int window_height=3, unsigned window_width_=3);
    ...
    void save_disparity(const char* out_file_name);
  ...
  private:
      ...
      vector<vector<float>> inv_confidence_;
